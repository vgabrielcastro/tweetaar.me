...
export const Sidebar = () => {
  ...
  return (
    <>
        ...
        <div className="!mt-auto space-y-3">
          ...
          <address className="pb-4 text-platinum not-italic">
            ...
            <a
              ...
              href="https://github.com/vgabrielcastro" target="_blank" rel="noreferrer noopener"
            >
              @Vinicius-Gabriel
            </a>
          </address>
      ...
      <aside className="flex w-full space-x-3 lg:hidden">
        <Link
          to="/"
          className={`flex justify-center items-center w-1/4 max-h-32 aspect-square ${location.pathname === '/' ? 'bg-birdBlue' : 'bg-transparent'}`}
        >
          <div>
            {location.pathname === '/' ? (
              <HomeSolid className="w-8 m-0" />
            ) : (
              <HomeOutline className="w-8 m-0" />
            )}
          </div>
        </Link>
        ...
        <Link
          to="/bookmarks"
          className={`flex justify-center items-center w-1/4 max-h-32 aspect-square ${location.pathname === '/bookmarks' ? 'bg-birdBlue' : 'bg-transparent'}`}
        >
          <div>
            {location.pathname === '/bookmarks' ? (
              <BookmarkSolid className="w-8 m-0" />
            ) : (
              <BookmarkOutline className="w-8 m-0" />
            )}
          </div>
        </Link>
        ...
        <Link
          to="/profile"
          className={`flex justify-center items-center w-1/4 max-h-32 aspect-square ${location.pathname === '/profile' ? 'bg-birdBlue' : 'bg-transparent'}`}
        >
          <div>
            {location.pathname === '/profile' ? (
              <UserSolid className="w-8 m-0" />
            ) : (
              <UserOutline className="w-8 m-0" />
            )}
          </div>
        </Link>
        ...
        <button
          onClick={logout}
          className="flex justify-center items-center w-1/4 max-h-32 aspect-square"
        >
          <LogoutIcon className="w-8 m-0 stroke-birdBlue" />
        </button>
      </aside>
    </>
  )
